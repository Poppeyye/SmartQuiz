...
def add_score():
    ...
    existing_score = PlayerScore.query.filter_by(name=player_name, category=category).first()
    ...
    if existing_score:
        ...
        new_score = PlayerScore(name=player_name, score=new_score_value, date=datetime.now(), category=category)
...
def get_user_rank(category=None):
    ...
    if category:
        user_rank = db.session.query(PlayerScore).filter_by(category=category).filter(PlayerScore.score > user_score).count() + 1
    else:
        user_rank = db.session.query(PlayerScore).filter(PlayerScore.score > user_score).count() + 1
